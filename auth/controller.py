...
from common.models.response import ResponseResult, retResponseResult
...
# 새 API 키 생성
@auth_router.post(
        "/token", status_code=201,
        dependencies=[Depends(authorize_key_required)],
        response_model=ResponseResult, response_model_exclude_none=True
)
async def token_generate(api_key_create: api_key_create, authorize_key: str = Depends(authorize_key_required)) -> dict:
    ...
    if not res_code:
        ...
        return retResponseResult(result)

    result = ResponseResult(result_code=status_code, data=res_msg)
    return result
